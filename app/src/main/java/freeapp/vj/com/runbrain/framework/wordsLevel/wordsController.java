...
public class wordsController implements IGameController {
    ...
    public wordsController(Context context, int width, int height){
        ...
        int numero = sixRandom.nextInt(35);

        while (sixR.size() < 6){

            if(sixR.contains(numero)){
                numero = sixRandom.nextInt(35);
            }
            else{
                sixR.add(numero);
            }
        ...
        firstTime = true;
        firstPress = false;
        model = new wordsModel(width, height);
    ...
    public Bitmap onDrawingRequested() {
        ...
        if (firstTime){
            ...
            graphics.drawText("Memoriza las palabras de la lista y luego", width/8, height/2 - 200, Color.BLUE, 45);
            graphics.drawText("indica si la palabra de la pantalla estaba en la lista!", width/8 - 90, height/2 - 160, Color.BLUE, 45);
            ...
            if(secFirst > 0){
                ...
                for(int i = 0; i<3; i++){
                    int x = width / 2;
                    int y = height-250;
                    graphics.drawText(""+model.palabra(i), (width / 2) - 380, 550 + (290 * i), Color.BLACK, 50);
                    model.addTO(x, y);
                }
                ...
                for(int i = 3; i<6; i++){
                    int x = width / 2;
                    int y = height-250;
                    graphics.drawText(""+model.palabra(i), (width / 2) + 100, 550 + (290 * a), Color.BLACK, 50);
                    model.addTO(x, y);
                    a++;
                }
...
*/
                ...
                if(botonU == true || firstPress) {
                    ...
                    random = new Random();
                    aleatorio = random.nextInt(35);
                    graphics.drawText(""+model.palabra(aleatorio), (width / 2)-170, height/2, Color.BLACK, 90);
        ...
        if(sec <=0){
            ...
            if(aciertosHigh < aciertos){
                ...
                editor.putInt("wordAciertos", aciertos);
                editor.putInt("wordFallos", fallos);
                editor.apply();
            }
